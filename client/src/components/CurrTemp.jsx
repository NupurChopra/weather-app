  ...
  return (
    <div className="text-white">
      <div className="flex bg-slate-800 rounded-xl text-white justify-between w-full h-48 gap-4">
        <div className="flex-1">
          ...
          <h3 className="text-gray-500 font-semibold px-6 pt-2">
            {/* Chances of Rain: {element.clouds?.all || "...."}% */}
      ...
      <div className='flex px-6 mt-10 mb-6 justify-between'>
        <p><span className="text-slate-400 font-semibold">Max Temperature:</span> {temp.maxTemp} °{unit}</p>
        <p><span className="text-slate-400 font-semibold">Min Temperature:</span> {temp.minTemp} °{unit}</p>
        <p><span className="text-slate-400 font-semibold">Avg Temperature: </span>{((Number(temp.temp_max)+Number(temp.temp_min))/2).toFixed(2)} °{unit}</p>
      </div>
      <p className="px-6"><span className="text-slate-400 font-semibold">Dormant weater condition :</span> {element.weather[0]?.description}</p>
      ...
      <div className="w-full bg-slate-800  h-30  mt-8 px-6 pb-4 rounded-xl">
        ...
        <div className=" grid grid-cols-2 gap-4">
          <div className="  px-5 pt-2">
            <h3 className="text-slate-400 flex  items-baseline gap-2 text-base font-semibold"><span className="text-xl"><FaThermometerHalf /></span>Real Feel</h3>
            <h1 className="text-2xl px-6 font-bold">{temp.feelslike} °{unit}</h1>
          </div>
          <div className="  px-5 pt-2">
            <h3 className="text-slate-400 flex items-baseline gap-2 text-base font-semibold"><span className="text-xl"><FaWind /></span>Wind</h3>
            <h1 className="text-2xl px-6 font-bold">{temp.wind} km/h</h1>
          </div>
          <div className="  px-5 pt-2">
            <h3 className="text-slate-400 flex items-baseline gap-2 text-base font-semibold"><span className="text-xl"><FaDroplet /></span>Humidity</h3>
            <h1 className="text-2xl px-6 font-bold">{element.main.humidity}</h1>
          </div>
          <div className="  px-5 pt-2">
            <h3 className="text-slate-400 flex items-baseline gap-2 text-base font-semibold"><span className="text-xl"><MdWindPower /></span>Air Pressure</h3>
